...
static void fillmap(const std::filesystem::path& path, std::vector<std::filesystem::path>& paths) {
...
static int rosetta(const Process& proc, int argc, const char* argv[]) {
...
ADD_TOOL("rosetta", " [data]", "rosetta", false, rosetta);
