    ...
    namespace {
        ...
        */
        ...
        bool IsCustomZone(const char* zoneName) {
            return std::find_if(zoneHooks.zoneNames.begin(), zoneHooks.zoneNames.end(), [zoneName](const char* zn) {
                       return !_strcmpi(zoneName, zn);
        ...
        void DB_LoadXAssets(XZoneInfo* zoneInfo, int zoneCount, bool sync) {
            ...
            for (size_t i = 0; i < zoneCount; i++) {
                ...
                if (it == zoneHooks.hooks.end())
                    continue; // nothing to hook
                ...
                try {
                    ...
                    if (itHooks != cfg.main.MemberEnd() && itHooks->value.IsArray()) {
                        ...
                        for (rapidjson::Value& hook : itHooks->value.GetArray()) {
                            ...
                            // to test, not for use
                            bool isCommon = isCommonIt == obj.MemberEnd() || !isCommonIt->value.IsBool() ||
                                            isCommonIt->value.GetBool();
