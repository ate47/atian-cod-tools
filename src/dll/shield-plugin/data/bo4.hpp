...
namespace bo4 {
    ...
    constexpr fieldtype_t GetFieldType() {
        if constexpr (std::is_same_v<T, int32_t>)
            ...
            return F_SHORT;
        else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, char> || std::is_same_v<T, bool>)
            return F_BYTE;
        else if constexpr (std::is_same_v<T, float>)
            return F_FLOAT;
        else if constexpr (std::is_same_v<T, XHash>)
            return F_HASH;
    ...
    enum BuiltinType : uint32_t { BUILTIN_DEFAULT = 0, BUILTIN_DEV = 1 };
