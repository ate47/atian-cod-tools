...
namespace core::config {
    ...
    ConfigGenericRefs::operator bool() const { return !base.IsNull(); }
    ...
    int64_t ConfigGenericRefs::GetInteger(const char* path, int64_t defaultValue) { return defaultValue; }

    uint64_t ConfigGenericRefs::GetUnsignedInteger(const char* path, uint64_t defaultValue) { return defaultValue; }

    double ConfigGenericRefs::GetDouble(const char* path, double defaultValue) { return defaultValue; }

    const char* ConfigGenericRefs::GetCString(const char* path, const char* defaultValue) { return defaultValue; }
    ...
    bool ConfigGenericRefs::GetBool(const char* path, bool defaultValue) { return defaultValue; }
    ...
    ConfigGenericRefs::operator bool() const { return false; }

    void ConfigGenericRefs::SetEnum(const char* path, int64_t enumValue, const ConfigEnumData* data, size_t dataCount) {
    }

    bool Config::SyncConfig(bool save) { return false; }
    void Config::SaveConfig() const {}

#endif // __ACTS_COMPRESS_HAS_RAPIDJSON

    void SetMainConfig(const std::filesystem::path& path) { mainConfigFile = path; }

    const std::filesystem::path& GetMainConfigFile() { return mainConfigFile; }
