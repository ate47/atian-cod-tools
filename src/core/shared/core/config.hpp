...
namespace core::config {
    ...
    class ConfigGenericRefs {
      public:
        ...
        inline RapidJsonGeneric& GetVal(const char* path) {
            return GetVal(path, 0, base);
        }
        ...
        inline int64_t GetInteger(const std::string& path, int64_t defaultValue = 0) {
            return GetInteger(path.c_str(), defaultValue);
        }

        uint64_t GetUnsignedInteger(const char* path, uint64_t defaultValue = 0);
        inline uint64_t GetUnsignedInteger(const std::string& path, uint64_t defaultValue = 0) {
            return GetUnsignedInteger(path.c_str(), defaultValue);
        }
        ...
        inline double GetDouble(const std::string& path, double defaultValue = 0) {
            return GetDouble(path.c_str(), defaultValue);
        }
        const char* GetCString(const char* path, const char* defaultValue = nullptr);
        inline const char* GetCString(const std::string& path, const char* defaultValue = nullptr) {
            return GetCString(path.c_str(), defaultValue);
        }
        inline const char* GetCString(const char* path, const std::string& defaultValue) {
            return GetCString(path, defaultValue.c_str());
        }
        inline const char* GetCString(const std::string& path, const std::string& defaultValue) {
            return GetCString(path.c_str(), defaultValue.c_str());
        }
        ...
        inline std::string GetString(const std::string& path, const char* defaultValue = "") {
            return GetString(path.c_str(), defaultValue);
        }
        inline std::string GetString(const char* path, const std::string& defaultValue) {
            return GetString(path, defaultValue.c_str());
        }
        inline std::string GetString(const std::string& path, const std::string& defaultValue) {
            return GetString(path.c_str(), defaultValue.c_str());
        }
        ...
        inline void SetInteger(const std::string& path, int64_t defaultValue) {
            SetInteger(path.c_str(), defaultValue);
        }
        void SetUnsignedInteger(const char* path, uint64_t defaultValue);
        inline void SetUnsignedInteger(const std::string& path, uint64_t defaultValue) {
            SetUnsignedInteger(path.c_str(), defaultValue);
        }
        ...
        inline void SetString(const std::string& path, const std::string& defaultValue) {
            SetString(path.c_str(), defaultValue);
        }
    ...
    class ConfigGeneric : public ConfigGenericRefs {
      public:
        ...
        template<typename T, typename Output = size_t, bool nullValid = true, size_t maxCount>
    ...
    constexpr auto CreateConfigEnumDataArray() {
        ...
        for (size_t i = 0; i < names.size(); i++) {
            table[i] = ConfigEnumData{ names[i].data(), static_cast<int64_t>(values[i]) };
        }
...
#else
            static_assert(always_false_v<T>, "Unsupported enum type without magic_enum");
#endif
        }
    };
    ...
    class ConfigDocument : public ConfigGeneric<DefaultTypeGetters> {
      public:
        ...
        RapidJsonDocument main{};
#endif
        ConfigDocument() : ConfigGeneric<DefaultTypeGetters>(main, main) { SetConfigPath(GetMainConfigFile()); }

        ConfigDocument(const std::filesystem::path& path) : ConfigGeneric<DefaultTypeGetters>(main, main) {
            SetConfigPath(path);
        }
        ConfigDocument(const ConfigDocument& other)
            : ConfigGeneric<DefaultTypeGetters>(this->main, this->main), configFile(other.configFile) {
#ifdef __ACTS_COMPRESS_HAS_RAPIDJSON
        ...
        ConfigDocument(ConfigDocument&& other) noexcept
            : ConfigGeneric<DefaultTypeGetters>(main, main), configFile(std::move(other.configFile)) {
#ifdef __ACTS_COMPRESS_HAS_RAPIDJSON
    ...
    inline void SetInteger(const char* path, int64_t value) { GetMainConfig().SetInteger(path, value); }
    inline void SetInteger(const std::string& path, int64_t value) { GetMainConfig().SetInteger(path, value); }

    inline void SetUnsignedInteger(const char* path, int64_t value) { GetMainConfig().SetUnsignedInteger(path, value); }
    inline void SetUnsignedInteger(const std::string& path, int64_t value) {
        GetMainConfig().SetUnsignedInteger(path, value);
    }
    inline void SetDouble(const char* path, double value) { GetMainConfig().SetDouble(path, value); }
    inline void SetDouble(const std::string& path, double value) { GetMainConfig().SetDouble(path, value); }
    inline void SetString(const char* path, const std::string& value) { GetMainConfig().SetString(path, value); }
    inline void SetString(const std::string& path, const std::string& value) { GetMainConfig().SetString(path, value); }
    inline void SetBool(const char* path, bool value) { GetMainConfig().SetBool(path, value); }
    inline void SetBool(const std::string& path, bool value) { GetMainConfig().SetBool(path, value); }
     ...
     */
    inline bool SyncConfig(bool save) { return GetMainConfig().SyncConfig(save); }
    /*
     * Save the config
     */
    inline void SaveConfig() { GetMainConfig().SaveConfig(); }
} // namespace core::config