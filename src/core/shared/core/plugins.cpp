...
namespace core::plugins {
    ...
    void LoadPlugins() {
        ...
        std::filesystem::path pluginsDir{ plugins.pluginsDir.empty() ? utils::GetProgDir() / "plugins"
                                                                     : plugins.pluginsDir };
        ...
        for (const std::filesystem::path& plugin : pluginFiles) {
            ...
            uint32_t maxPluginVersion{ pGetActsMaxVersion ? pGetActsMaxVersion()
                                                          : GetNextMajorVersion(minPluginVersion) };
