...
namespace core::bytebuffer {
    ...
    class ByteBuffer : public AbstractByteBuffer {
      ...
      public:
        ...
        bool CanRead(size_t size) const override { return pointer + size <= len; }

        size_t Length() const override { return len; }

        size_t Remaining() const override { return len - pointer; }
        size_t Loc() const override { return pointer; }
        ...
        std::vector<size_t> FindAllMasked(void* ptr, void* mask, size_t ptrSize) {
            ...
            while (curr + ptrSize <= len) {
                ...
                while (delta < ptrSize) {
                    if ((buffer[curr + delta] & ((byte*)mask)[delta]) != ((byte*)ptr)[delta])
                        break;
                    delta++;
                }
                if (delta == ptrSize) {
