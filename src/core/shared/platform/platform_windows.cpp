...
namespace platform {
    ...
    int32_t LoadSharedFlagsToPlatform(int32_t flags) {
        ...
        if (flags & LSF_SEARCH_DEFAULT_DIRS)
            nflags |= LOAD_LIBRARY_SEARCH_DEFAULT_DIRS;
        if (flags & LSF_DONT_RESOLVE_REFERENCES)
            nflags |= DONT_RESOLVE_DLL_REFERENCES;
    ...
    void* GetModulePointer(const char* mod) { return GetModuleHandleA(mod); }
    ...
    byte* BasePtr() { return reinterpret_cast<byte*>(BaseHandle()); }

    PIMAGE_DOS_HEADER PImageDosHeader(void* mod) {
        return reinterpret_cast<PIMAGE_DOS_HEADER>(mod ? mod : BaseHandle());
    }

    PIMAGE_NT_HEADERS PImageNtHeader(void* mod) { return ImageNtHeader((mod ? mod : BaseHandle())); }

    PIMAGE_OPTIONAL_HEADER PImageOptHeader(void* mod) { return &(PImageNtHeader(mod)->OptionalHeader); }
    ...
    const char* GetDebugPath(void* hmod) {
        ...
        if (utils::ReadFile(GetSharedPath(hmod), lib)) {
            ...
            if (lib.size() < dosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS) ||
    ...
    std::vector<const char*> GetIATModules(const hook::library::Library& lib) {
        ...
        if (!dir.VirtualAddress || dir.Size <= 0)
            return res; // nothing
    ...
    bool FreeShared(void* lib) { return FreeLibrary((HMODULE)lib); }
    ...
    namespace {
        ...
        class NearContainer {
          ...
          public:
            ...
            constexpr bool CanAllocate(size_t len) const { return used + len <= capacity; }
            ...
            bool IsCloseEnough(void* location, size_t len = 1) const {
                return Int32Distance((byte*)location + used, container) &&
                       Int32Distance((byte*)location + used + len, container);
            }

            bool CanAllocateClose(void* location, size_t len) const {
                return CanAllocate(len) && Int32Distance((byte*)location + used, container) &&
                       Int32Distance((byte*)location + used + len, container);
            }
        };
        ...
        SYSTEM_INFO& GetSysInfo() {
            ...
            std::call_once(pageSizeFlag, [] { GetSystemInfo(&sysInfo); });
            return sysInfo;
        }
    ...
    void* AllocateNearPage(void* location) {
        ...
        while (true) {
            ...
            if (highAddr < maxAddr) {
                void* outAddr =
                    VirtualAlloc((void*)highAddr, pageSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
                if (outAddr) {
                    return outAddr;
                }
            }
            ...
            if (lowAddr > minAddr) {
                void* outAddr =
                    VirtualAlloc((void*)lowAddr, pageSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
                if (outAddr != nullptr) {
                    return outAddr;
                }
            }
    ...
    std::vector<MemoryRegion> EnumerateMemoryRegions(void* mod) {
        ...
        while (current < moduleEnd) {
            ...
            if (regionEnd > regionStart) {
                regions.push_back({ regionStart, static_cast<size_t>(regionEnd - regionStart), readable });
            }
    ...
    void DumpStackTraceFrom(core::logs::loglevel level, const void* location) {
        ...
        if (location) {
            for (; i < capture; i++) {
                if (locs[i] == location)
                    break; // skip until we reach our location
            }
        }
        if (i == capture) {
            i = 0;
    ...
    uint32_t GetLastPlatformError() { return GetLastError(); }
    ...
    class WinModuleInformation : public ModuleInformation {
      public:
        ...
        size_t SizeOfImage() override { return PImageOptHeader(hmod)->SizeOfImage; }

        size_t ImageBase() override { return PImageOptHeader(hmod)->ImageBase; }
