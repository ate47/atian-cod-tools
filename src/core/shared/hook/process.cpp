...
namespace hook::process {
    ...
    void* LoadSysLib(const char* lib) { return platform::LoadSystemShared(lib); }
    ...
    asmjit::Error AssemblerExp::call(void* target) { return Assembler::call(size_t(target)); }

    asmjit::Error AssemblerExp::jmp(void* target) { return Assembler::jmp(size_t(target)); }
#endif
} // namespace hook::process