...
namespace hook::error {
    ...
    ErrorConfig& GetErrorConfig() { return cfg; }
    ...
    void InstallErrorHooks(bool clearSetFunction) { platform::InstallErrorHooks(clearSetFunction); }

    void AddErrorDumper(void (*dumper)()) { cfg.dumpers.push_back(dumper); }
    ...
    void DevSetContinue(bool cont) { cfg.cont = cont; }
} // namespace hook::error