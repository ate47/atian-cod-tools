...
namespace hook::library {
    void* GetLibraryInfo(const void* address) { return platform::GetModuleOfAddress(address); }

    const char* GetLibraryName(void* hmod) { return platform::GetSharedName(hmod); }

    const char* GetLibraryPath(void* hmod) { return platform::GetSharedPath(hmod); }

    const char* LocatePDB(void* hmod) { return platform::GetDebugPath(hmod); }
    ...
    const char* CreateScanPatternString(const char* str) { return CreateScanPattern(str, std::strlen(str) + 1); }
    ...
    std::vector<ScanResult> ScanLibrary(void* hmod, const char* pattern, bool single, const char* name) {
        ...
        size_t delta{};
        hook::scan_loader::PackScan(pattern, mask, searched, delta, name);
        ...
        for (platform::MemoryRegion& region : platform::EnumerateMemoryRegions(hmod)) {
            ...
            for (size_t off = 0; off + mask.size() <= regionSize; off++) {
                ...
                if (match) {
                    res.emplace_back(current + off - delta);
                    if (single) {
    ...
    struct ScanContainer {
        ...
        std::filesystem::path GetPath() { return utils::va("%s.scan", name); }
    };
    ...
    void InitScanContainer(const char* name) {
        ...
        if (*reinterpret_cast<decltype(SCAN_CONTAINER_FILE_MAGIC)*>(file) !=
            (SCAN_CONTAINER_FILE_MAGIC | SCAN_CONTAINER_FILE_VERSION)) {
    ...
    void ScanLogger::WriteLogs(std::filesystem::path out, ScanLoggerLogsOpt* opt) {
        ...
        }
        {
            ...
            idaScript << "#include <idc/idc.idc>\n\n";
            if (opt->headerWriter)
                opt->headerWriter(idaScript);
            ...
            if (opt->inMainWriter)
                opt->inMainWriter(idaScript);
            idaScript << "}\n";
        }
    }

    void ScanLogger::Clean() { entries.clear(); }
    ...
    std::vector<const char*> hook::library::Library::GetIATModules() const { return platform::GetIATModules(*this); }
