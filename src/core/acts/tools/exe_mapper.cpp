...
namespace {
        ...
        while (imports->Name) {
            ...
            while (originalThunks->u1.Function) {
                ...
                if (originalThunk.u1.Ordinal & IMAGE_ORDINAL_FLAG64) {
                    outVC << std::hex << "0x" << originalThunk.u1.Ordinal << "-> "
                          << hook::library::CodePointer{ &thunk } << "\n";
                } else {
                    outVC << lib.Get<const char>(originalThunk.u1.Function + 2) << "-> "
                          << hook::library::CodePointer{ &thunk } << "\n";
                }
            }
        }
        ...
        for (const std::filesystem::path& path : paths) {
            ...
            if (animtcount) {
                ...
                for (; j < animtcount; j++) {
                    ...
                    if (rloc + sizeof(*animt) + sizeof(uint32_t) * animt->num_address > buffer.size() ||
                        animt->address_str1 >= buffer.size() || animt->address_str2 >= buffer.size()) {
            ...
            if (animtucount) {
                ...
                for (; k < animtucount; k++) {
                    ...
                    if (rloc + sizeof(*animtu) + sizeof(uint32_t) * animtu->num_address > buffer.size() ||
                        animtu->address >= buffer.size()) {
    ...
    constexpr bool IsValidChar(char c) { return (c >= 0x20 && c < 0x7F) || c == '\n' || c == '\r' || c == '\t'; }
