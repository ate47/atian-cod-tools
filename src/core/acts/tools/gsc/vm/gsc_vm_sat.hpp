...
#include <tools/gsc/data/gsc_data_t10.hpp>

namespace tool::gsc {
    template<tool::gsc::opcode::VMId Magic>
    class T1116GSCOBJHandler : public GSCOBJHandler {
    public:
        T1116GSCOBJHandler(byte* file, size_t fileSize) : GSCOBJHandler(file, fileSize, GOHF_ANIMTREE | GOHF_ANIMTREE_DOUBLE | GOHF_FOREACH_TYPE_JUP | GOHF_NOTIFY_CRC_XHASH | GOHF_SUPPORT_EV_HANDLER | GOHF_SUPPORT_VAR_VA | GOHF_VAR_VA_COUNT | GOHF_SWITCH_TYPE_IW) {}

        void DumpHeaderInternal(std::ostream& asmout, const GscInfoOption& opt) override {
            GscObj24* data = Ptr<GscObj24>();
            const char* crchash{ utils::va("%d", static_cast<int32_t>(data->checksum)) };
            hashutils::AddPrecomputed(hash::Hash64(crchash), crchash, false, true);
        ...
        opcode::Platform ComputePlatform(T8GSCOBJContext& ctx) override {
            ...
            if (!GetExportsCount()) {
                return PLATFORM_PC; // no exports, so no value to check
            }
