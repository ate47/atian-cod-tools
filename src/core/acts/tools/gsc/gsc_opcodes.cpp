...
namespace {
    ...
    static struct {
        ...
        { "unk", "Unknown" },     { "pc", "PC" },     { "ps", "PlayStation" },          { "xbox", "Xbox" },
        { "pc_alpha", "Alpha" },  { "test", "Test" }, { "bo6_v1", "Black Ops 6 (V1)" }, { "bo6", "Black Ops 6" },
        { "bo7", "Black Ops 7" },
    };
...
namespace tool::gsc::opcode {
    ...
    const char* PlatformName(Platform plt) { return platformNames[plt < PLATFORM_COUNT ? plt : PLATFORM_UNKNOWN].name; }

    const char* PlatformIdName(Platform plt) { return platformNames[plt < PLATFORM_COUNT ? plt : PLATFORM_UNKNOWN].id; }

    void VmInfo::AddDevCallName(uint64_t name) { devCallsNames.insert(name & hash::MASK60); }
    void VmInfo::SetCompilerHookFunctionName(const char* name) {
        uint64_t hash{ HashField(name) };
        if (compilerHookFunctionName && compilerHookFunctionName != hash)
            LOG_WARNING("compiler hook function defined twice for {}", name);
    ...
    uint64_t VmInfo::HashFilePath(const char* value) const { return HashField(value); }

    bool VmInfo::IsScrHash(uint64_t hash) const {
        if (HasFlag(VMF_UNIQUE_HASH))
            return false;
    ...
    void VmInfo::SetModToolFlag(uint16_t flag) { this->modToolFlag = flag; }

    void VmInfo::SetOpaqueStringCount(size_t opaqueStringCount) { this->opaqueStringCount = opaqueStringCount; }
    ...
    void VmInfo::RegisterDatatype(const char* datatype) { dataType[hash::Hash64(datatype)] = datatype; }
