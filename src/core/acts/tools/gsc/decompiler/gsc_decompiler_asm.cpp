...
namespace tool::gsc::opcode {
    ...
    bool IsEqualNumber(ASMContextNode* a, ASMContextNode* b) {
        ...
        switch (dt) {
        case ASCNVD_FLOAT:
            return dynamic_cast<ASMContextNodeValue<FLOAT>*>(a)->m_value ==
                   dynamic_cast<ASMContextNodeValue<FLOAT>*>(b)->m_value;
        case ASCNVD_INT:
    ...
    bool IsStructSimilar(ASMContextNode* a, ASMContextNode* b) {
        ...
        case TYPE_GET_CODE_REF:
            return dynamic_cast<ASMContextNodeCodeRef*>(a)->m_location ==
                   dynamic_cast<ASMContextNodeCodeRef*>(b)->m_location;
        case TYPE_IDENTIFIER:
            return dynamic_cast<ASMContextNodeIdentifier*>(a)->m_value ==
                   dynamic_cast<ASMContextNodeIdentifier*>(b)->m_value;
        case TYPE_VECTOR: {
            ...
            return IsStructSimilar(veca->m_x, vecb->m_x) && IsStructSimilar(veca->m_y, vecb->m_y) &&
                   IsStructSimilar(veca->m_z, vecb->m_z);
        }
        case TYPE_TERNARY: {
            ...
            return IsStructSimilar(tera->m_operand1, terb->m_operand1) &&
                   IsStructSimilar(tera->m_operand2, terb->m_operand2) &&
                   IsStructSimilar(tera->m_operand3, terb->m_operand3);
        }
        ...
        case TYPE_OPERATION_MERGE: {
            ...
            return IsStructSimilar(aa->m_operand1, bb->m_operand1) && IsStructSimilar(aa->m_operand2, bb->m_operand2);
        }
        case TYPE_ARRAY_ACCESS: {
            ...
            return IsStructSimilar(aa->m_operandleft, bb->m_operandleft) &&
                   IsStructSimilar(aa->m_operandright, bb->m_operandright);
        }
        case TYPE_ACCESS:
        case TYPE_SET: {
        ...
        case TYPE_SET_OP: {
            ...
            return aa->m_operatorName == bb->m_operatorName && IsStructSimilar(aa->m_left, bb->m_left) &&
                   IsStructSimilar(aa->m_right, bb->m_right);
        }
        case TYPE_FUNC_REFNAME: {
            ...
            return aa->m_func == bb->m_func && aa->m_script == bb->m_script && aa->m_nsp == bb->m_nsp &&
    ...
    bool IsBlockType(tool::gsc::opcode::ASMContextNodeType type) {
        ...
        return type == TYPE_BLOCK || type == TYPE_DO_WHILE || type == TYPE_WHILE || type == TYPE_FOR ||
               type == TYPE_FOR_EACH || type == TYPE_IF || type == TYPE_SWITCH_POSTCOMPUTE ||
               type == TYPE_SWITCH_PRECOMPUTE;
    }
    ...
    ASMContextNodeBlock::ASMContextNodeBlock(ASMContextNodeBlockType blockType, bool disabled, bool allowInline)
        : ASMContextNode(PRIORITY_INST, TYPE_BLOCK), m_blockType(blockType), m_disabled(disabled),
          m_allowInline(allowInline) {
    ...
    ASMContextNode::ASMContextNode(ASMContextNodePriority priority, ASMContextNodeType type)
    ...
    int64_t ASMContextNode::GetIntConst() const { return 0; }
    ...
    void ASMContextLocation::RemoveRef(int32_t origin) { refs.erase(origin); }
    ...
    std::ostream& ASMContext::WritePadding(std::ostream& out) {
        if (m_opt.m_func_floc) {
            out << "." << std::hex << std::setfill('0') << std::setw(sizeof(int32_t) << 1)
                << (m_bcl - m_gscReader.Ptr());
        }
        return out << "." << std::hex << std::setfill('0') << std::setw(sizeof(int32_t) << 1)
    ...
    UINT ASMContext::FinalSize() const {
        ...
        return (UINT)max +
               (this->m_objctx.m_vmInfo->HasFlag(VmFlags::VMF_OPCODE_U16) ? 2 : 1); // +1 for the Return/End operator
    }
    ...
    void ASMContext::ComputeDefaultParamValue() {
        ...
        while (it != m_funcBlock.m_statements.end()) {
            ...
            if (param1->m_type == TYPE_JUMP_ONTRUE) {
                ...
                if (isDefinedFunc->m_operand->m_type != TYPE_IDENTIFIER || set->m_left->m_type != TYPE_IDENTIFIER ||
                    !set->m_right) {
                ...
                if (localVar == -1 || localVar < m_localvars.size() - 1 - m_exp.GetParamCount() ||
                    m_localvars[localVar].defaultValueNode) {
                ...
                if (jump->m_operand->m_type != TYPE_IDENTIFIER || set->m_left->m_type != TYPE_IDENTIFIER ||
                    !set->m_right) {
                    break; // not isdefined(param_name) or not param_name = ...
                }
                uint64_t name = static_cast<ASMContextNodeIdentifier*>(set->m_left)->m_value;
    ...
    void DecompContext::MarkLine(std::ostream& out) {
        if (gdbData && baseloc && lineBuf) {
            ...
            if (l < lineRloc) {
                ...
                if (opt.m_lineCount) {
                    out << "/*" << std::dec << lineBuf->line << "/" << std::hex << std::setfill('0')
                        << std::setw(sizeof(int32_t) << 1) << (lineRloc) << "*/";
                }
            }
        }
    }
    ...
    void ASMContextNodeBlock::Dump(std::ostream& out, DecompContext& ctx) const {
        ...
        size_t i{};
        size_t visibleId{};
        enum { LT_NONE = 0, LT_BLOCK, LT_STMT } lastType{};
        while (i < m_statements.size()) {
            ...
            if (ref.location->refs.size() && ref.node->m_renderRefIfAny) {
                ...
                ctx.WritePadding(out, true)
                    << "LOC_" << std::hex << std::setfill('0') << std::setw(sizeof(int32_t) << 1) << ref.location->rloc;
            ...
            if (i < m_statements.size() && hide && !ctx.opt.m_show_internal_blocks) {
                    ...
                    do {
                            if (m_statements[i].location->refs.size()) {
                                    // this element is referenced, it might be from the front, a later add would be to
            check
                                    // if by looking back.
                                    hide = false;
                                    break;
    ...
    int ASMContextNodeBlock::ComputeSwitchBlocks(ASMContext& ctx) {
        ...
        while (it != m_statements.end()) {
            ...
            ASMContextNodeSwitchPostCompute* switchBlock =
                new ASMContextNodeSwitchPostCompute(switchPreCompute->m_node->Clone());
            ...
            if (cases.size()) {
                ...
                do {
                    ASMContextNodeBlockType blockType =
                        (ctx.m_opt.m_formatter->HasFlag(tool::gsc::formatter::FFL_SWITCH_FORCE_BLOCKS)) ? BLOCK_DEFAULT
                                                                                                        : BLOCK_PADDING;
    ...
    void ApplySubStatement(
        ...
        if (stmt.node->m_type != TYPE_BLOCK) {
            ...
            stmt.node->ApplySubBlocks(
                [endOnReturn, &func](ASMContextNodeBlock* block, ASMContext& ctx) {
        ...
        dynamic_cast<ASMContextNodeBlock*>(stmt.node)->ApplySubBlocks(
            [&func, endOnReturn](ASMContextNodeBlock* block, ASMContext& ctx) {
                for (auto& stmt : block->m_statements) {
                    ApplySubStatement(stmt, ctx, func, endOnReturn);
                    if (endOnReturn && stmt.node &&
                        (stmt.node->m_type == TYPE_RETURN || stmt.node->m_type == TYPE_END)) {
                        break;
                    }
                }
            },
        ...
        */
        ...
        while (index < m_statements.size()) {
            ...
            if (arrayRefLoc.node->m_type != TYPE_SET ||
                (setOp = static_cast<ASMContextNodeLeftRightOperator*>(arrayRefLoc.node))->m_left->m_type !=
                    TYPE_IDENTIFIER) {
                index += moveDelta;
                continue;
            }
            ...
            *(namesEnd++) = arrayRefName;
            ...
            if (arrayDefinedTest.node->m_type != TYPE_JUMP_ONFALSE ||
                (arrayDefinedTestJump = static_cast<ASMContextNodeJumpOperator*>(arrayDefinedTest.node))
                        ->m_operand->m_type != TYPE_FUNC_IS_DEFINED_OP ||
                arrayDefinedTestJump->m_delta < 0 ||
                (arrayDefinedTestJumpIsDefined = static_cast<ASMContextNodeMultOp*>(arrayDefinedTestJump->m_operand))
                        ->m_operands.size() != 1 ||
                arrayDefinedTestJumpIsDefined->m_operands[0]->m_type != TYPE_IDENTIFIER ||
                static_cast<ASMContextNodeIdentifier*>(arrayDefinedTestJumpIsDefined->m_operands[0])->m_value !=
                    arrayRefName) {
                index += moveDelta;
                continue;
            }
            ...
            if (getArrayKeys.node->m_type != TYPE_SET ||
                (setKeysOp = static_cast<ASMContextNodeLeftRightOperator*>(getArrayKeys.node))->m_left->m_type !=
                    TYPE_IDENTIFIER ||
                setKeysOp->m_right->m_type != TYPE_FUNC_CALL ||
                (funcCall = static_cast<ASMContextNodeCallFuncPtr*>(setKeysOp->m_right))
                    ->m_flags                       // no flags = normal call
            ...
            *(namesEnd++) = keysRefName;
            ...
            if (incInit.node->m_type != TYPE_SET ||
                (setIncOp = static_cast<ASMContextNodeLeftRightOperator*>(incInit.node))->m_left->m_type !=
                    TYPE_IDENTIFIER ||
                !setIncOp->m_right->IsConstNumber() || setIncOp->m_right->GetIntConst()) {
                index += moveDelta;
                continue;
            }
            ...
            *(namesEnd++) = incRefName;
            ...
            if (getKeyStmt.node->m_type != TYPE_SET ||
                (getKey = static_cast<ASMContextNodeLeftRightOperator*>(getKeyStmt.node))->m_right->m_type !=
                    TYPE_ARRAY_ACCESS ||
                getKey->m_left->m_type != TYPE_IDENTIFIER ||
                (keyAccess = static_cast<ASMContextNodeArrayAccess*>(getKey->m_right))->m_operandleft->m_type !=
                    TYPE_IDENTIFIER ||
            ...
            *(namesEnd++) = keyRefName;
            ...
            if (getValueStmt.node->m_type != TYPE_SET ||
                (getValue = static_cast<ASMContextNodeLeftRightOperator*>(getValueStmt.node))->m_right->m_type !=
                    TYPE_ARRAY_ACCESS ||
                getValue->m_left->m_type != TYPE_IDENTIFIER ||
                (valueAccess = static_cast<ASMContextNodeArrayAccess*>(getValue->m_right))->m_operandleft->m_type !=
                    TYPE_IDENTIFIER ||
            ...
            *(namesEnd++) = valueRefName;
            ...
            if ((checkValueStmt.node->m_type != TYPE_JUMP_ONFALSE &&
                 checkValueStmt.node->m_type != TYPE_IF)) { // assume ok, if for empty block
                index += moveDelta;
                continue;
            }
            ...
            while (true) {
                ...
                if (maybeSet.node->m_type != TYPE_SET ||
                    (setOpRem = static_cast<ASMContextNodeLeftRightOperator*>(maybeSet.node))->m_left->m_type !=
                        TYPE_IDENTIFIER ||
            ...
            // we found the foreach, we can now patch the jumps (TODO: find if we need to specify the jump location,
            // i.e.: nested foreach)
            ...
            ASMContextNodeForEach* forEachNode =
                new ASMContextNodeForEach(setOp->m_right->Clone(), block, keyUsed ? keyRefName : 0, valueRefName);
        ...
        */
        ...
        while (index < m_statements.size()) {
            ...
            */
            ...
            ASMContextNodeLeftRightOperator* setFirstArrayOp =
                static_cast<ASMContextNodeLeftRightOperator*>(firstArrayLoc.node);
            ...
            if (([setFirstArrayOp]() {
                    ...
                    ASMContextNodeCallFuncPtr* callFunc =
                        dynamic_cast<ASMContextNodeCallFuncPtr*>(setFirstArrayOp->m_right);

                    if (callFunc->m_flags || !callFunc->m_operands.size() ||
                        callFunc->m_operands[0]->m_type != TYPE_FUNC_REFNAME) {
                        return true;
                    }
                    ...
                    return funcRef->m_func != getfirstarraykeyIWHash && funcRef->m_func != getfirstarraykeyCerHash &&
                           funcRef->m_func != getfirstarraykeyCerBHash;
                })()) {
                index += moveDelta;
            ...
            */
            ...
            size_t forincsize = ctx.m_objctx.m_vmInfo->HasFlag(VmFlags::VMF_FOREACH_IW) ? 2
                                : ctx.m_vm <= VMI_T8_36                                 ? 3
                                                                                        : 4;
            ...
            if (ctx.m_objctx.m_vmInfo->HasFlag(VmFlags::VMF_FOREACH_IW)) {
                /*
                        ...
                        continue
                */
                ASMContextNodeLeftRightOperator* setKeyArrayOp =
                    static_cast<ASMContextNodeLeftRightOperator*>(itemSetStmt.node);
                ...
                */
                ...
                ASMContextNodeLeftRightOperator* setKeyArrayOp =
                    static_cast<ASMContextNodeLeftRightOperator*>(itemSetStmt.node);
                ASMContextNodeLeftRightOperator* setValArrayOp =
                    static_cast<ASMContextNodeLeftRightOperator*>(iteratorvalStmt.node);
                ASMContextNodeLeftRightOperator* setNextArrayOp =
                    static_cast<ASMContextNodeLeftRightOperator*>(iteratornextStmt.node);
                ...
                */
                ...
                if (nextArrayStmt.node->m_type != TYPE_ARRAY_NEXTKEY &&
                    !(nextArrayStmt.node->m_type == TYPE_SET &&
                      static_cast<ASMContextNodeLeftRightOperator*>(nextArrayStmt.node)->m_right->m_type ==
            ...
            */
            ...
            if (jumpBackStmt.node->m_type != TYPE_END && jumpBackStmt.node->m_type != TYPE_RETURN &&
                (jumpBackStmt.node->m_type != TYPE_JUMP ||
                ...
                if (it != m_statements.end()) {
                    ...
                    if (it->node->m_type == TYPE_SET) {
                        ...
                        if (clearKeyVal->m_left->m_type == TYPE_IDENTIFIER &&
                            ((tmp = static_cast<ASMContextNodeIdentifier*>(clearKeyVal->m_left))->m_value ==
                                 keyValName ||
                             tmp->m_value == arrayRefName) &&
    ...
    int ASMContextNodeBlock::ComputeWhileBlocks(ASMContext& ctx) {
        ...
        LOC_000001d6:
                LOC_00000208:jumpiffalse(self isgrappling() && self getcurrentoffhand() == weapon) LOC_00000256 delta:
        0x4a; self waittill(#"hash_347a612b61067eb3"); self bot_action::function_8a2b82ad(actionparams); self
        bot_action::aim_at_target(actionparams); LOC_00000252:goto LOC_000001d6 delta: -0x80; LOC_00000256:
            ...
            if (jumpOp->m_type == TYPE_JUMP) {
                ...
                for (size_t i = startIndex; i < index; i++) {
                    ...
                    if (IsJumpType(ref->m_type)) {
                        ...
                        ref->ApplySubBlocks(
                            [&](ASMContextNodeBlock* block, ASMContext& context) {
                                for (const auto& stmt : block->m_statements) {
                                    ...
                                    if (IsJumpType(refjump->m_type)) {
                                        ...
                                        if (j->m_location == breakLoc) {
                                            if (!j->m_special_op) {
                                                j->m_operatorName =
                                                    context.m_opt.m_mark_jump_type ? "break<inf_block>" : "break";
                                                j->m_special_op = SPECIAL_JUMP_BREAK;
                                                j->m_showJump = false;
                                            }
                                            ...
                                            //}
                                                   ...
                                                   j->m_special_op != SPECIAL_JUMP_BREAK) {
                                            if (!j->m_special_op) {
                                                j->m_operatorName = context.m_opt.m_mark_jump_type
                                                                        ? "continue<inf_block_end_it>"
                                                                        : "continue";
                                                j->m_special_op = SPECIAL_JUMP_CONTINUE;
                                                j->m_showJump = false;
                                            }
        ...
        */
        ...
        while (index < m_statements.size()) {
            ...
            if (endIndex > index + 1 && m_statements[endIndex - 1].node->m_type == TYPE_JUMP) {
                ...
                // after else goto
                if (jumpOpEndElse->m_delta > 0 && jumpOpEndElse->m_special_op == SPECIAL_JUMP_DEFAULT &&
                    !jumpOpEndElse->m_returnCandidate) {
                    auto endElseLoc = jumpOpEndElse->m_location;
                    // should be positive and not a special jump

                    auto endElseIndex = endIndex;
                    while (endElseIndex < m_statements.size() &&
                           m_statements[endElseIndex].location->rloc < endElseLoc) {
                        endElseIndex++;
                    }
            ...
            ASMContextNode* ifCond =
                JumpCondition(static_cast<ASMContextNodeJumpOperator*>(m_statements[index].node), true);
            if (!(ctx.m_opt.m_formatter->HasFlag(tool::gsc::formatter::FFL_NO_BOOL_ANALYSIS))) {
        ...
        // LOC_0000017c delta: 0x1c (type:9/spec:false)
    ...
    bool IsForDeltaCandidate(ASMContextNode* node, uint64_t incrementName) {
        if (node->m_type == TYPE_DELTA) {
            ...
            return deltaPart->m_operand->m_type == TYPE_IDENTIFIER &&
                   static_cast<ASMContextNodeIdentifier*>(deltaPart->m_operand)->m_value == incrementName;
        }
        if (node->m_type == TYPE_SET) {
            // i = ????
            auto* setOp = static_cast<ASMContextNodeLeftRightOperator*>(node);

            return setOp->m_left->m_type == TYPE_IDENTIFIER &&
                   static_cast<ASMContextNodeIdentifier*>(setOp->m_left)->m_value == incrementName;
        }
        return false;
    }
    ...
    namespace {
        void ApplyPreSpecialPatternBlock(ASMContext& ctx, std::vector<ASMContextStatement>& stmts) {
            /* // Replace ondefined jump to default_to, but it's not really important and probably annoying to add in
            the compiler for (size_t i = 0; i < stmts.size(); i++) { auto& stmt = stmts[i];

                    stmt.node->ApplySubBlocks([](ASMContextNodeBlock* block, ASMContext& ctx) {
            ApplyPreSpecialPatternBlock(ctx, block->m_statements); }, ctx);

                    if (!(ctx.m_opt.m_stepskip & tool::gsc::STEPSKIP_SPECIAL_PATTERN)) {
                            if (i < 2 || stmts[i - 1].node->m_type != TYPE_SET || stmts[i - 2].node->m_type !=
            TYPE_JUMP_ONDEFINED) { continue;
                            }

                            ASMContextNodeLeftRightOperator* set =
            dynamic_cast<ASMContextNodeLeftRightOperator*>(stmts[i - 1].node); ASMContextNodeJumpOperator* jump =
            dynamic_cast<ASMContextNodeJumpOperator*>(stmts[i - 2].node);

                            if (!IsStructSimilar(set->m_left, jump->m_operand) || jump->m_location !=
            stmt.location->rloc) { continue;
                            }
        ...
        void ApplySpecialPatternBlock(ASMContext& ctx, std::vector<ASMContextStatement>& stmts) {
            for (size_t i = 0; i < stmts.size(); i++) {
                ...
                if (!(ctx.m_opt.m_stepskip & tool::gsc::STEPSKIP_SPECIAL_PATTERN)) {
                    ...
                    if (ctx.m_objctx.m_vmInfo->HasFlag(VmFlags::VMF_IW_LIKE) && i &&
                        stmts[i - 1].node->m_type == TYPE_SET &&
                        (mainSet = dynamic_cast<ASMContextNodeLeftRightOperator*>(stmts[i - 1].node))->m_left->m_type ==
                            TYPE_IDENTIFIER &&
                        ...
                        */
                        ...
                        int32_t keyrefs;
                        if (keyit != ctx.m_localvars_ref.end() && (keyrefs = keyit->second) > 1 &&
                            i + keyrefs - 1 <= stmts.size()) {
                            size_t j{ i };
                            ...
                            for (; j < end; j++) {
                                ...
                                ASMContextNodeLeftRightOperator* set =
                                    dynamic_cast<ASMContextNodeLeftRightOperator*>(stmts[j].node);
                                if (set->m_right->m_type != TYPE_ARRAY_ACCESS) {
                                    break; // not: var = ... [ ... ]
                                }
                                ASMContextNodeArrayAccess* arrayAccess =
                                    dynamic_cast<ASMContextNodeArrayAccess*>(set->m_right);
                                if (arrayAccess->m_operandleft->m_type != TYPE_IDENTIFIER ||
                                    !arrayAccess->m_operandright->IsIntConst() ||
                                    dynamic_cast<ASMContextNodeIdentifier*>(arrayAccess->m_operandleft)->m_value !=
                                        keyHash) {
                                    break; // not: var = arr [ x ]
                                }
                                ...
                                if (idx < 0)
                                    break; // can't have negative index
                            ...
                            if (latest == 0) {
                                ...
                                for (size_t k = 1; k < keyrefs; k++) {
                                    ASMContextNodeLeftRightOperator* set =
                                        dynamic_cast<ASMContextNodeLeftRightOperator*>(stmts[i + (keyrefs - k)].node);
                                    assert(set != nullptr);
                    ...
                    if (i
                        ...
                        */
                        && stmt.node->m_type == TYPE_SET &&
                        dynamic_cast<ASMContextNodeLeftRightOperator*>(stmt.node)->m_right->m_type == TYPE_WAITTILL &&
                        stmts[i - 1].node->m_type == TYPE_SET &&
                ...
                if (!(ctx.m_opt.m_stepskip & tool::gsc::STEPSKIP_DEVBLOCK_INLINE) && stmt.node->m_type == TYPE_BLOCK) {
                    // inline dev calls
                    ...
                    if (db->m_blockType == BLOCK_DEV && SizeNoEmptyNode(db->m_statements) == 1 &&
                        GetNoEmptyNode(db->m_statements, 0)->node->m_type == TYPE_FUNC_CALL) {
                        ...
                        if (fc && fc->m_ftype == FUNCTION_CALL) {
                            ...
                            if (name->m_type == TYPE_FUNC_REFNAME) {
                                ...
                                if (ctx.m_objctx.m_vmInfo->devCallsNames.find(refName) !=
                                    ctx.m_objctx.m_vmInfo->devCallsNames.end()) {
                                    // inline the dev call
                                    stmt.node = fc;
                                    dbStmt->node = nullptr;
                                    delete db;
                ...
                if (ctx.m_exp.GetName() == constr) {
                    for (size_t i = 0; i < stmts.size();) {
                        ...
                        if (stmt.node->m_type == TYPE_FUNC_CALL) {
                            ...
                            if (fc->m_ftype == FUNCTION_CALL && !(fc->m_flags & SELF_CALL) &&
                                fc->m_operands[0]->m_type == TYPE_FUNC_REFNAME &&
    ...
    int ASMContextNodeBlock::ComputeCustomCompilerPattern(ASMContext& ctx) {
        ...
        for (size_t i = 0; i < m_statements.size(); i++) {
            ...
            ApplySubStatement(stmt, ctx, [compilerHookFunctionName, &ctx](ASMContextStatement& stmt) {
                ...
                if (fc->m_operands.size() < 2 || fc->m_operands[0]->m_type != TYPE_FUNC_REFNAME ||
                    (fc->m_operands[1]->m_type != TYPE_VALUE && fc->m_operands[1]->m_type != TYPE_CONST_HASH)) {
