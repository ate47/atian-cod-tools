...
namespace tool::gsc::opcode {
    ...
    struct FunctionOperator {
        ...
        constexpr bool HasFlag(uint64_t flag) { return (flags & flag) == flag; }
    };
    ...
    struct VmInfo {
         ...
         */
        constexpr bool HasFlag(uint64_t flag) const { return (flags & flag) == flag; }
        ...
        inline uint64_t HashField(const std::string& value) const { return HashField(value.c_str()); }
        inline uint64_t HashPath(const std::string& value) const { return HashPath(value.c_str()); }
        inline uint64_t HashFilePath(const std::string& value) const { return HashFilePath(value.c_str()); }
        ...
        inline void RegisterVmName(const char* name) { RegisterVmName(hash::Hash64(name)); }
