...
namespace hash {
    ...
    namespace {
        ...
        const std::string DefaultVal(uint64_t h) { return h ? std::format("{:x}", h) : "N/A"; }
        ...
        void hashsearch_nui() {
            ...
            auto LoadHashes = [] {
                ...
                utils::GetFileRecurse(loadPath, files, [](const std::filesystem::path& p) {
                    ...
                    return name.ends_with(".gsc") || name.ends_with(".csc") || name.ends_with(".csv") ||
                           name.ends_with(".tsv") || name.ends_with(".dec.lua") || name.ends_with(".json");
                });
        ...
        int Render(HWND window, HINSTANCE hInstance) {
            ...
            if (info.hashEdit == NULL || info.hashEditLabel == NULL || info.hash64Edit == NULL ||
                ...
                info.hash64IW2Edit == NULL || info.hash64IW2EditLabel == NULL || info.hash64IW3Edit == NULL ||
                info.hash64IW3EditLabel == NULL || info.hash32IW4Edit == NULL || info.hash32IW4EditLabel == NULL ||
                info.hashT7Edit == NULL || info.hashT7EditLabel == NULL || info.hash64IWDVEdit == NULL ||
                info.hash64IWDVEditLabel == NULL || info.hash64IWCerFieldEdit == NULL ||
                info.hash64IWCerFieldEditLabel == NULL || info.hashFNV1AReverseValEdit == NULL ||
                info.hashFNV1AReverseIVEdit == NULL || info.hashFNV1AReverseValLabel == NULL ||
                info.hashFNV1AReverseStartEdit == NULL || info.hashFNV1AReverseStartEditLabel == NULL ||
                info.hashFNV1ACustomEdit == NULL || info.hashFNV1ACustomEditLabel == NULL ||
                info.hashLookupEdit == NULL || info.hashLookupEditLabel == NULL || info.hashLookupEditRet == NULL ||
        ...
        LRESULT Update(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
            if (uMsg == WM_COMMAND) {
                if (B_HIWORD(wParam) == EN_CHANGE) {
                    if (info.hashEdit == (HWND)lParam || info.hashFNV1AReverseValEdit == (HWND)lParam ||
                        info.hashFNV1AReverseIVEdit == (HWND)lParam) {
                        ComputeHashes();
                    }
                ...
                if (lParam == (LPARAM)info.hashEdit || lParam == (LPARAM)info.hashEditLabel ||
                    lParam == (LPARAM)info.hash64Edit || lParam == (LPARAM)info.hash64EditLabel ||
                    lParam == (LPARAM)info.hash32Edit || lParam == (LPARAM)info.hash32EditLabel ||
                    lParam == (LPARAM)info.hash64IW2Edit || lParam == (LPARAM)info.hash64IW2EditLabel ||
                    lParam == (LPARAM)info.hash64IW3Edit || lParam == (LPARAM)info.hash64IW3EditLabel ||
                    lParam == (LPARAM)info.hash32IW4Edit || lParam == (LPARAM)info.hash32IW4EditLabel ||
                    lParam == (LPARAM)info.hashT7Edit || lParam == (LPARAM)info.hashT7EditLabel ||
                    lParam == (LPARAM)info.hash64IWDVEdit || lParam == (LPARAM)info.hash64IWDVEditLabel ||
                    lParam == (LPARAM)info.hash64IWCerFieldEdit || lParam == (LPARAM)info.hash64IWCerFieldEditLabel ||
                    lParam == (LPARAM)info.hashFNV1AReverseValEdit || lParam == (LPARAM)info.hashFNV1AReverseIVEdit ||
                    ...
                    lParam == (LPARAM)info.hashFNV1ACustomEdit || lParam == (LPARAM)info.hashFNV1ACustomEditLabel ||
                    lParam == (LPARAM)info.hashLookupEdit || lParam == (LPARAM)info.hashLookupEditLabel ||
                    lParam == (LPARAM)info.hashLookupEditRet || lParam == (LPARAM)info.titleLabel) {
        ...
        int fnv1acrackcommon(int argc, const char* argv[]) {
            ...
            tool::hash::text_expand::GetDynamic<Ctx>(
                ctx.max,
                [](const char* s, Ctx* ctx) {
                    ctx->s1 = s;
        ...
        int bruteforcecustom(int argc, const char* argv[]) {
            tool::hash::text_expand::GetDynamicAsync<void>(std::string::npos, [](const char* s, void* ctx) {
                ...
                if (p == 0xa16ec18512a89f20) {
                    LOG_INFO("find2 {}", s);
                } else if (p == 0xa2556a2905fd952) {
                    LOG_INFO("finda255 {}", s);
                }
                // else if (p == 0x7a42b57be462143f) { LOG_INFO("find {}", s); } // Demspey Matryoshka Dolls
                else if (p == 0x291e1c117ebbf5e6) {
                    LOG_INFO("find29 {}", s);
                } // Richtofen Matryoshka Dolls / rich
                else if (p == 0x7264d6f24a950a5b) {
                    LOG_INFO("find72 {}", s);
                } // Nikolai Matryoshka Dolls / niko
                else if (p == 0x579652e2459b8c74) {
                    LOG_INFO("find57 {}", s);
                } // Takeo Matryoshka Dolls / takeo
            });
            return tool::OK;
        }
        ...
        int crctest(int argc, const char* argv[]) {
            ...
            for (size_t i = 2; i < argc; i++) {
                ...
                LOG_INFO("sha1:{} / sha256:{}", sha1.getHash(), sha256.getHash());
                LOG_INFO("sha1:{} / sha256:{}", sha1.getHash(), sha256.getHash());
                LOG_INFO("md5:{}", md5.getHash());
                uint64_t xx64{ XXH64(buff.data(), buff.size(), 0) };
                uint32_t xx64r{ (uint32_t)(xx64 ^ (xx64 >> 32)) };
                uint32_t xx32{ XXH32(buff.data(), buff.size(), 0) };

                LOG_INFO("xxh64:{:x} xxh64r:{:x} xxh32:{:x}", xx64, xx64r, xx32);
            }
...
uint32_t ActsAPIHash_DecodeErrorCode(const ActsAPIHash_ErrorCode* code) { return error_coder::Decode(*code); }

const char* ActsAPIHash_ErrorCodeToStr(const ActsAPIHash_ErrorCode* code) { return error_coder::ToStr(*code); }
