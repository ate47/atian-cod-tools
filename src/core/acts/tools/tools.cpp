...
namespace tool {
    ...
    std::map<std::string, tool::toolcategory>& toolsCategories() {
        ...
        if (map.empty()) {
            ...
            map["internal"] = { "internal", "Internal tools", false, {} };
        }
    ...
    bool toolfunctiondata::operator!() const { return !m_name; }

    bool toolfunctiondata::operatorbool() const { return !m_name; }

    const toolfunctiondata& findtool(const char* name) {
        constexpr auto invalidToolName = LOG_GET_LOG_REF_STR_DATA;
        static toolfunctiondata invalid{
            nullptr, invalidToolName.data(), (size_t)__LINE__, nullptr, nullptr, nullptr, nullptr, nullptr
        };
    ...
    bool search(const char** query, int paramCount, std::function<void(const toolfunctiondata* tool)> each) {
        ...
        for (const auto& [name, tool] : tools()) {
            ...
            for (const auto& [s, w] : args) {
                if (tool->m_nameLower.rfind(s) != std::string::npos ||
    ...
    const char** ReadParams(std::string& str, int& argc) { return container.ReadParams(str, argc); }
...
namespace {
    ...
    int help(int argc, const char* argv[]) { return list(argc, argv); }
    ...
    int color(int argc, const char* argv[]) {
        for (int b = 0; b < 6; b++) {
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 6; j++) {
                    std::cout << " " << cli::clicolor::ColorBackground(i, j, b) << i << j << b
                              << cli::clicolor::Reset();
                }
                std::cout << " ";
                for (int j = 0; j < 6; j++) {
