...
namespace {
    ...
    int luajit(int argc, const char* argv[]) {
        ...
        for (const std::filesystem::path& path : files) {
            ...
            if (filename.ends_with(".dec.lua"))
                continue; // ignore decompiled file
            ...
            if (opt.outdir) {
                ...
                if (hash::TryHashPattern(noextfn.c_str(), hash)) {
                    ...
                    if (!name)
                        name = utils::va("hashed/luafile/file_%llx.lua", hash);
            ...
            if (opt.header) {
                ...
                if (flags & LJF_IS_STRIPPED)
                    os << " stripped";
                if (flags & LJF_HAS_FFI)
                    os << " ffi";
                if (flags & LJF_HAS_FR2)
                    os << " fr2";
                if (flags & ~LJF_MASK)
                    os << " <missing>";
                os << "\n";
    ...
    */
    ...
    const char* ljErrors[]{
        "ok", "yield", "run error", "syntax error", "memory error", "error",
    };
    ...
    class ljec_Context {
      ...
      public:
        ...
        int Write(lua_State* L, const void* p, size_t sz) {
            if (!outDump)
                throw std::runtime_error("no file to dump");
        ...
        void PatchAntiCheat(hook::scan_container::ScanContainer& scan) {
            ...
            struct PatchInfo {
                ...
                { "49 3B C2 0F 82 ?? ?? ?? ?? 48 8B 84 24 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 49 3B C2 0F 87 ?? ?? ?? ?? "
                  "48 8B 84 24 ?? ?? ?? ?? 80 78 FB E8 74 ??",
                  46 },
                { "48 3b c1 0f 82 ?? ?? ?? ?? 48 8b 44 24 ?? 48 ?? ?? ?? ?? ?? ?? 48 3b c1 0f ?? ?? ?? ?? ?? 48 8b 44 "
                  "24 ?? 80 78 fb e8 74 ??",
                  40 },
            };
    ...
    void* ljec_Alloc(void* ud, void* ptr, size_t osize, size_t nsize) { return currentCtx->Alloc(ptr, osize, nsize); }

    void* ljec_Reader(lua_State* L, void* ud, size_t* sz) { return currentCtx->Reader(L, sz); }

    int ljec_Writer(lua_State* L, const void* p, size_t sz, void* ud) { return currentCtx->Write(L, p, sz); }
    ...
    int ljec(int argc, const char* argv[]) {
        ...
        hook::error::InstallErrorHooks(true);
        hook::module_mapper::Module mod{ true };
        std::filesystem::path modPath{ opt.gamePath ? opt.gamePath
                                                    : (utils::GetProgDir() / "deps" / ctx.data.GetModuleName()) };
