...
namespace fastfile {
    ...
    namespace {
        ...
        const char* gameIdNames[]{
            "invalid", "bo3", "iw", "wwii", "bo4", "mw19", "cw", "vg", "mwii", "mwiii", "bo6", "bo7",
        };
        ...
        void RegisterCrypts() {
            int r;
            if ((r = register_hash(&sha256_desc)) != CRYPT_OK)
                throw std::runtime_error(std::format("Can't register sha256_desc {}", error_to_string(r)));
            if ((r = register_cipher(&aes_desc)) != CRYPT_OK)
                throw std::runtime_error(std::format("Can't register aes_desc {}", error_to_string(r)));
            if ((r = crypt_mp_init("ltm")) != CRYPT_OK)
                throw std::runtime_error(std::format("Can't init ltm {}", error_to_string(r)));
        }
    ...
    fastfile::FastFileContext& GetCurrentContext() { return *currentCtx; }

    fastfile::FastFileOption& GetCurrentOptions() { return *currentOpt; }
    ...
    const char* FastFileContext::GetFFType() { return ActsAPIFastFile_FastFileContext_GetType(this); }
    ...
    void SetBP(void* ptr, byte len) { HwBp::Set(ptr, len, HwBp::When::Written); }
        ...
        void StartRepl() {
            ...
            do {
                ...
                if (cli::clicolor::ConsoleAllowColor()) {
                    std::cout << cli::clicolor::Color(5, 1, 3) << "acts:" << core::actsinfo::VERSION << "> "
                              << cli::clicolor::Color(5, 5, 5);
    ...
    int fastfilelinker(int argc, const char* argv[]) {
        ...
        if (!opt.Compute(argv, 2, argc) || opt.m_help ||
            (opt.files.empty() && !(opt.printCompressors || opt.printLinkers))) {
            opt.PrintHelp();
            if (opt.files.empty()) {
        ...
        for (const char* file : opt.files) {
            try {
                ...
                utils::CloseEnd compCE{ [&ctx]() {
                    if (ctx.compressor)
                        ctx.compressor->Cleanup();
                } };
