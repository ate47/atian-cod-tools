...
namespace fastfile {
    ...
    class FastFileOption : public ActsAPIFastFile_FastFileOption {
      public:
        ...
        inline bool ReadFile(const std::string& path, std::vector<byte>& buff) { return ReadFile(path.data(), buff); }
    ...
    class FFDecompressor {
      public:
        ...
        FFDecompressor(const char* name, uint64_t magic = 0, uint64_t mask = MASK64)
            : name(name), magic(magic & mask), mask(mask) {}
...
#define LOG_OPT_INFO(...)                                                                                              \
    if (!fastfile::GetCurrentOptions().reducedLogs)                                                                    \
