...
namespace {
    ...
    const char* CompressionNames[]{
        "none",        "zlib",        "zlib_hc",      "lz4",           "lz4_hc",       "bdelta_uncomp",
        "bdelta_zlib", "bdelta_lzma", "oodle_kraken", "oodle_mermaid", "oodle_selkie", "oodle_lzna",
    };
    struct XFile {
        uint8_t magic[8];
        uint32_t version;
        ...
        for (const char* f : opt.files) {
            for (const std::string filename : opt.GetFileRecurse(f)) {
                ...
                auto PrintXFile = [&opt, &ret, &reader](XFile* buffer) {
                    ...
                    if (buffer->encrypted) {
                        auto checksum32 =
                            fastfile::ComputeChecksum32(buffer->key, (unsigned int)strlen(buffer->key), 0);
                    ...
                    if (!opt.ReadFile(filenameFd.data(), fileFDBuff)) {
                        ...
                        if (!bo4.bdiff(
                                ...
                                [](uint64_t offset, uint64_t size) -> byte* {
                                    ...
                                    if (!ffDecode.buffbd->CanRead(size))
                                        return nullptr;
                    ...
                    */
                    ...
                    while (true) {
                        ...
                        std::filesystem::path outFile{ out /
                                                       std::format("vm_{:x}/script_{:x}.gscc", smagic, obj->name) };
    ...
    int ffexe(int argc, const char* argv[]) {
        ...
        for (const char* f : opt.files) {
            ...
            auto PrintXFile = [&ret, &reader](XFile* buffer) {
                ...
                if (buffer->encrypted) {
                    auto checksum32 = fastfile::ComputeChecksum32(buffer->key, (unsigned int)strlen(buffer->key), 0);
                    LOG_INFO("checksum32 . {}", checksum32);
                }
    ...
    int h32ck(Process& proc, int argc, const char* argv[]) {
        ...
        for (size_t i = 2; i < argc; i++) {
            auto chk = fastfile::ComputeChecksum32(argv[i], (unsigned int)strlen(argv[i]), 0);
