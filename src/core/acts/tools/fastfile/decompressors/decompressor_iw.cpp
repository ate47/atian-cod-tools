    ...
    void DumpTafHeader(utils::OutFileCE& hos, fastfile::flexible::FlexibleFastFileReader& freader, const char* type) {
        ...
        for (fastfile::flexible::FlexibleFastFileChunk& chunk : freader) {
            ...
            default: {
                ...
                switch (chunk.size) {
                case 1:
                    view = std::format("0x{:x}", (int)chunk.GetVal<byte>());
                    break;
                case 2:
                    view = std::format("0x{:x}", chunk.GetVal<uint16_t>());
                    break;
                case 4:
                    view = std::format("0x{:x}", chunk.GetVal<uint32_t>());
                    break;
                case 8:
                    view = std::format("0x{:x}", chunk.GetVal<uint64_t>());
                    break;
                default: {
                    if (chunk.size & 7) {
    ...
    class IWFFDecompressor : public fastfile::FFDecompressor {
      ...
      public:
        ...
        void Init(fastfile::FastFileOption& opt) override {
            // DB_UnpackHeader = game->ScanSingle("40 55 53 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 88
            // B4").GetPtr<decltype(DB_UnpackHeader)>(); game->Redirect("48 89 5C 24 ? 55 56 57 B8 ? ? ? ? E8 ? ? ? ? 48
            // 2B E0 48 8B 05 ? ? ? ? 48 33 C4 48 89 84 24 ? ? ? ? 48 8B EA", []() { return true; }); decryptBuffer =
        ...
        void LoadFastFile(
            ...
            if (opt.headerDump) {
                std::filesystem::path headerDumpLoc{ std::filesystem::path{ opt.headerDump } / ctx.GetFFType() /
                                                     std::format("{}.txt", ctx.ffname) };
            ...
            while (reader.CanRead(sizeof(uint32_t) * 3)) {
                ...
                if (secure && secureType == ST_MW22) {
                    if ((id & 0x1ff) == 0x1ff) {
                        if (!reader.CanRead(sizeof(FS_RSAChunk)))
                            break;
                ...
                fastfile::XBlockCompressionBlockHeader* block{
                    reader.ReadPtr<fastfile::XBlockCompressionBlockHeader>()
                };
