      ...
      public:
        ...
        void LoadFastFile(
            ...
            if (trPlatform.encrypted) {
                ...
                int stat{};
                // if ((r = rsa_verify_hash(trFastfileInfo.signature, 0x100, digest, sizeof(digest), shaHash, 8, &stat,
                // &rsakey)) != CRYPT_OK) {
                //	throw std::runtime_error(std::format("Hash verify error {} for ff {}", error_to_string(r),
                // ctx.ffname));
                // }
