    ...
    class T78FFDecompressor : public fastfile::FFDecompressor {
      public:
        ...
        void LoadFastFile(
            ...
            if (opt.m_header) {
                LOG_INFO(
                    "{} fastfile version 0x{:x}, encrypted:{}, platform: {}, builder: {}",
                    fastfile::GetFastFileCompressionName(header->compression),
                    header->version,
                    header->encrypted ? "true" : "false",
                    fastfile::GetFastFilePlatformName(header->platform),
                    header->builder
                );
            }
            ...
            if (header->encrypted) {
                ...
                if (!aesIV) {
                    throw std::runtime_error(
                        std::format("Missing decrypt data for this fast file version 0x{:x}", header->version)
                    );
                }
                ...
                if (rsaKeyName) {
                    compatibility::acti::crypto_keys::RsaKeyLocal* rsa{
                        compatibility::acti::crypto_keys::GetRSAKeyByName(rsaKeyName)
                    };
                    if (rsa) {
                        ...
                        if (shaHash == -1) {
                            throw std::runtime_error(
                                std::format("Missing sha256 for ff {}/{}", ctx.ffname, rsaKeyName)
                            );
                        }
            ...
            while (offset < decompressedSize) {
                ...
                if (noStreamInfo) {
                    ...
                    if (header->encrypted) {
                        ...
                        if ((r = ctr_start(aesCipher, aesIV, aeskey->key, sizeof(aeskey->key), 0, 0, &ctr)) !=
                            CRYPT_OK) {
                            throw std::runtime_error(
                                std::format(
                                    "Failed to start ctr {} for ff {}/{}",
                                    error_to_string(r),
                                    ctx.ffname,
                                    rsaKeyName
                                )
            ...
            if (opt.m_fd) {
                ...
                if (opt.ReadFile(fdfile.string(), fileFDBuff)) {
                    ...
                    if (opt.m_header) {
                        LOG_INFO(
                            "s:0x{:x}, version:{}, flags:0x{:x} 0x{:x} 0x{:x} 0x{:x}, decompSize:0x{:x}",
                            bdiffHeader->size,
                            bdiffHeader->version,
                            bdiffHeader->flags,
                            bdiffHeader->maxDestWindowSize,
                            bdiffHeader->maxSourceWindowSize,
                    ...
                    if (bdiffHeader->size != bdiffHeaderSize) {
                        throw std::runtime_error(
                            std::format(
                                "Can't read patch file header size: 0x{:x} != 0x{:x}",
                                bdiffHeader->size,
                                bdiffHeaderSize
                            )
                        );
                    ...
                    if (bdiffHeader->maxDiffWindowSize < 0x10000)
                        bdiffHeader->maxDiffWindowSize = 0x10000;
                    if (bdiffHeader->maxDestWindowSize < 0x10000)
                        bdiffHeader->maxDestWindowSize = 0x10000;
                    if (bdiffHeader->maxSourceWindowSize < 0x10000)
                        bdiffHeader->maxSourceWindowSize = 0x10000;

                    size_t compressedSize{ fdreader.Remaining() };

                    LOG_TRACE(
                        "Decompressing patch {} 0x{:x}: 0x{:x} -> 0x{:x}",
                        fastfile::GetFastFileCompressionName(newXFileHeader->compression),
                        fdreader.Loc(),
                        compressedSize,
                        fdDecompressedSize
                    );
                    ...
                    default:
                        throw std::runtime_error(
                            std::format("No fastfile decompressor for type {}", (int)newXFileHeader->compression)
                        );
                    }
