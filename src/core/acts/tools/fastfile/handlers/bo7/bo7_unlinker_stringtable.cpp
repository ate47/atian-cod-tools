...
namespace {
					...
					*os << std::endl;
					for (size_t j = 0; j < asset->columnCount; j++) {
						...
						if (!elemSize) {
							...
							LOG_WARNING("INVALID TYPE FOR STRINGTABLE: 0x{:x}", (int)column->type);
							continue; // wtf?
						}
						...
						case STT_STRING: {
							...
							if (hook::memory::ReadMemorySafe((void*)str, &b, sizeof(b))) {
								...
								LOG_WARNING("INVALID STRING FOR STRINGTABLE");
							}
							break;
						}
						case STT_XHASH:
	...
	utils::MapAdder<ImplWorker, SatHashAssetType, Worker> impl{ GetWorkers(), SatHashAssetType::SATH_ASSET_STRINGTABLE, sizeof(StringTable) };
