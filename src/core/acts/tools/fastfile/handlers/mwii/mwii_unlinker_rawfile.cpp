...
namespace {
	...
	class ImplWorker : public Worker {
		...
		void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
			...
			LOG_OPT_INFO("Dump raw file {} type: 0x{:x} len: 0x{:x}", outFile.string(), asset->type, asset->uncompressedLen);
	...
	utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> impl{ GetWorkers(), HandlerHashedAssetType::IW9H_ASSET_RAWFILE, sizeof(RawFile) };
}