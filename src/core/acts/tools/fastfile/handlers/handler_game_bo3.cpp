...
namespace {
    ...
    const char* XFileBlockName(XFileBlock id) {
        static const char* names[]{ "TEMP",    "RUNTIME_VIRTUAL", "RUNTIME_PHYSICAL", "DELAY_VIRTUAL", "DELAY_PHYSICAL",
                                    "VIRTUAL", "PHYSICAL",        "STREAMER_RESERVE", "STREAMER",      "MEMMAPPED" };
...
#define ThrowFastFileError(...) ThrowFastFileError_(std::format(__VA_ARGS__))
    ...
    class BO3FFHandler : public fastfile::FFHandler {
      public:
        BO3FFHandler()
            : fastfile::FFHandler("bo3", "Black Ops 3", compatibility::scobalula::csi::CordycepGame::CG_BO3) {}
        ...
        void Handle(
            ...
            *gcx.varScriptStringList = &gcx.assetList;
            ...
            std::filesystem::path outStrings{ opt.m_output / "bo3" / "source" / "tables" / "data" / "strings" /
                                              std::format("{}.txt", ctx.ffname) };
            std::filesystem::path outAssets{ opt.m_output / "bo3" / "source" / "tables" / "data" / "assets" /
                                             std::format("{}.csv", ctx.ffname) };
            std::filesystem::path outXStrings{ gcx.opt->m_output / "bo3" / "source" / "tables" / "data" / "xstrings" /
                                               std::format("{}.txt", ctx.ffname) };
