...
namespace fastfile::handlers::cw {
    ...
    const char* XFileBlockName(XFileBlock id) {
        static const char* names[]{
            "TEMP",     "TEMP_PRELOAD", "RUNTIME_VIRTUAL", "RUNTIME_PHYSICAL", "VIRTUAL",   "CPU_GPU_SHARE",
            "PHYSICAL", "STREAMER",     "STREAMER_CPU",    "MEMMAPPED",        "LOAD_TIME", "TEMP_PRELOAD_SAVED_PTR",
            "MESH",
        };
    ...
    std::ostream& operator<<(std::ostream& os, const fastfile::handlers::cw::ScrString_t& scr) { return os << scr.id; }
    ...
    namespace {
        ...
        void Load_XStringCustom(char** str) {
            ...
            // XBlockLocPtr((void*)str), size, gcx.reader->Loc()); LOG_TRACE("- {}", utils::data::AsHex(ptr, size + 1));
        ...
        void* PMem_Alloc(size_t size, size_t alignment, void* loaded, int pool, int stack) {
            ...
            if (pool == 2)
                return &empty; // no gpu streamer pool
        ...
        class BOCWFFHandler : public fastfile::FFHandler {
          public:
            ...
            void Handle(
                ...
                std::filesystem::path outAssets{ gcx.opt->m_output / "cw" / "source" / "tables" / "data" / "assets" /
                ...
                if (assetList.assets) {
                    ...
                    if (opt.dumpXStrings) {
                        std::filesystem::path outStrings{ gcx.opt->m_output / "cw" / "source" / "tables" / "data" /
                                                          "xstrings" / std::format("{}.txt", ctx.ffname) };
                        std::filesystem::create_directories(outStrings.parent_path());
