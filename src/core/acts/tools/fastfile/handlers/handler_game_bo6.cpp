...
namespace fastfile::handlers::bo6 {
	...
	namespace {
		...
		public:
			...
			FFHandlerImpl() : fastfile::FFHandler(handlerId, handlerName, compatibility::scobalula::csi::CordycepGame::CG_BO6) {
			}
			...
			void Handle(fastfile::FastFileOption& opt, core::bytebuffer::ByteBuffer& ffData, fastfile::FastFileContext& ctx) override {
				gcx.ctx = &ctx;
				...
				LOG_DEBUG("string end at 0x{:x}", gcx.reader->Loc());

				gcx.assets.unk10 = gcx.reader->ReadPtr<uint32_t>(gcx.assets.unk10_count);
				// idk
