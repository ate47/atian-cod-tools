...
namespace fastfile::handlers::mwiii {
    ...
    namespace {
        ...
        struct {
            ...
            size_t loaded{};
            std::unordered_map<HandlerHashedAssetType, std::unordered_map<uint64_t, void*>>
                linkedAssets{}; // todo: use me
            AssetList assets{};
...
#define ThrowFastFileError(...) ThrowFastFileError_(std::format(__VA_ARGS__))
        ...
        class FFHandlerImpl : public fastfile::FFHandler {
          public:
            FFHandlerImpl()
                : fastfile::FFHandler(handlerId, handlerName, compatibility::scobalula::csi::CordycepGame::CG_MW6) {}
    ...
    const char* GetPoolName(uint32_t hash) { return gcx.assetNames.GetMappedType((HandlerHashedAssetType)hash)->name; }
    ...
    HandlerHashedAssetType GetHashType(HandlerAssetType type) { return gcx.assetNames.GetHashType(type); }

    HandlerAssetType GetExePoolId(const char* name) { return gcx.assetNames.GetExePoolId(name); }
    HandlerAssetType GetExePoolId(HandlerHashedAssetType name) { return gcx.assetNames.GetExePoolId(name); }
