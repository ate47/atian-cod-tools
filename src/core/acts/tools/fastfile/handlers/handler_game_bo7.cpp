...
namespace fastfile::handlers::bo7 {
    ...
    namespace {
        ...
        static_assert(sizeof(XZoneTemporaryLoadData) == 0x18e40);
        ...
        DBLoadCtxVT dbLoadCtxVTable{ DBLoadCtxUnk0, AllocStreamPos, PushStreamPos,
                                     PopStreamPos,  PreAssetRead,   PostAssetRead };
        ...
        struct {
            ...
            core::hashes::names::NamesStore namesStore{ [](const char* name) -> uint64_t {
                return hash::HashIWAsset(name);
            } };
        ...
        void Load_String(DBLoadCtx* context, char** pstr) {
            ...
            if (gcx.xstringLocs)
                gcx.xstringLocs->push_back(*pstr);
            if (**pstr == '#') {
        ...
        class FFHandlerImpl : public fastfile::FFHandler {
          public:
            ...
            FFHandlerImpl()
                : fastfile::FFHandler(handlerId, handlerName, compatibility::scobalula::csi::CordycepGame::CG_BO7) {}
            ...
            void Handle(
                ...
                if (!ffData.CanRead(sizeof(gcx.assets))) {
                    LOG_WARNING("empty fastfile, ignored");
                    return;
                }

                core::bytebuffer::ByteBuffer reader{ ffData.GetBuffer(), ffData.Length() };

                if (opt.workflow == FFW_ASSET_POOL) {
                    // we need to allocate the memory

                    byte* ffdata{ ctx.zoneMemory.Alloc<byte>(ffData.Length()) };
                    ffData.Read(ffdata, ffData.Length());
                    reader = { ffdata, ffData.Length() };
                }
                gcx.reader = &reader;
                ...
                std::filesystem::path outLoadedStrings{ gcx.opt->m_output / gamePath / "source" / "tables" / "data" /
                                                        "loaded_strings" / fftype / std::format("{}.csv", ctx.ffname) };
...

                {
                    ...
                    for (size_t i = 0; i < gcx.assets.stringsCount; i++) {
                        ...
                        stringsOs << std::dec << std::setfill(' ')
                                  << std::setw(utils::Log<10>(gcx.assets.stringsCount) + 1) << i << "\t"
                                  << std::setw(16) << std::setfill('0') << std::hex << hash << "\t" << str << "\n";
                    }
                }
                ...
                std::filesystem::path outAssets{ gcx.opt->m_output / gamePath / "source" / "tables" / "data" /
                                                 "assets" / fftype / std::format("{}.csv", ctx.ffname) };
                {
                ...
                if (gcx.xstringLocs) {
                    std::filesystem::path ostr{ gcx.opt->m_output / gamePath / "source" / "tables" / "data" /
                                                "xstrings" / fftype / std::format("{}.txt", ctx.ffname) };
    ...
    std::vector<const char*>* GetXStrings() { return gcx.xstringLocs; }

    const char* GetScrString(ScrString_t id) { return fastfile::GetScrString(id.id); }
