    ...
    class GameData {
      ...
      public:
        ...
        Type GetPointer(const char* id, const char* parent = BASE_PARENT) {
            ...
            switch (data.type) {
            case SCT_ABSOLUTE:
                return res.GetPtr<Type>(data.offset + data.postOffset);
            case SCT_RELATIVE:
                return res.GetRelative<int32_t, Type>(data.offset, data.postOffset);
            case SCT_GET_OFFSET32:
                return (Type)scan.GetLibrary().Get<Type>(res.Get<uint32_t>(data.offset) + data.postOffset);
            default:
                throw std::runtime_error(std::format("SCAN DATA TYPE NOT IMPLEMENTED : {}", (int)data.type));
            }
        }
