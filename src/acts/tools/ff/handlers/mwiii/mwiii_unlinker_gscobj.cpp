...
namespace {
	...
	class ImplWorker : public Worker {
		...
		void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
			...
			std::filesystem::path outFile{ opt.m_output / gameDumpId / "gscobj" / std::format("script_{:x}.gscc", asset->name) };
			...
			if (!opt.disableScriptsDecomp) {
				...
				std::filesystem::path outSourceGDB{ opt.m_output / gameDumpId / "source" / "gdb" };
				...
				std::filesystem::path outSource{ opt.m_output / gameDumpId / "source" };
	...
	class ImplWorkerGdb : public Worker {
		...
		void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
			...
			std::filesystem::path outFile{ opt.m_output / gameDumpId / "gscobj" / std::format("script_{:x}.gsc.gdb", asset->name) };
	...
	utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> impl{ GetWorkers(), HandlerHashedAssetType::JUPH_ASSET_GSCOBJ, sizeof(GscObj) };
	utils::MapAdder<ImplWorkerGdb, HandlerHashedAssetType, Worker> implGdb{ GetWorkers(), HandlerHashedAssetType::JUPH_ASSET_GSCGDB, sizeof(GscGdb) };
}