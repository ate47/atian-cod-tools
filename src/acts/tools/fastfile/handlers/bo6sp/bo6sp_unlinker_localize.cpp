...
namespace {
	...
	class ImplWorker : public Worker {
		...
		void PostXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override {
			...
			std::filesystem::path outFile{ opt.m_output / "bo6sp" / "localize" / std::format("{}.json", ctx.ffname) };
	...
	utils::MapAdder<ImplWorker, bo6::T10HashAssetType, Worker> impl{ GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_LOCALIZE, sizeof(Localize) };
}