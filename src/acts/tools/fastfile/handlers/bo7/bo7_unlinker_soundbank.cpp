...
namespace {
	...
	static_assert(sizeof(SndBankAlias) == 0x30);
				...
				for (size_t i = 0; i < asset->count; i++) {
					...
					if (aliases->aliases) {
						...
						for (size_t j = 0; j < aliases->count; j++) {
							...
							json.WriteFieldValueXAsset("sndAsset", SATH_ASSET_SNDASSET, alias->sndAsset);
							if (opt.testDump) {
								json.WriteFieldValueUnknown("alias:unk0", alias->unk0);
								json.WriteFieldValueNumber("alias:unk30", alias->unk28);
								json.WriteFieldValueUnknown("alias:unk34", alias->unk2c);
							}
							...
							if (alias->info) {
								...
								json.WriteFieldValueXAsset("soundSpeakerMap", SATH_ASSET_SOUNDSPEAKERMAP, info->soundSpeakerMap);
								...
								json.WriteFieldValueXAsset("sndOcclusionSetting", SATH_ASSET_SNDOCCLUSIONSETTING, info->sndOcclusionSetting);
							...
							for (size_t j = 0; j < aliases->count; j++) {
								...
								if (alias->text) {
									os
										...
										<< "/#" << hashutils::ExtractTmp("hash", GetXAssetName(SATH_ASSET_SNDASSET, alias->sndAsset))
										<< ") "
	...
	utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> implt{ GetWorkers(), SATH_ASSET_SOUNDBANKTRANSIENT, true };
}